...
int main()
{
    ...
    std::cout << " [>] 彩虹六号线下修改器（孤狼猎恐等专用）线上开挂biss。"<< "\n";
    std::cout << " [>] 本修改器由DineMX基于Oly474编写的Base制作，完全免费。\n";
    std::cout << " [>] 使用前务必在steam或Uplay启动项选项下输入/belaunch。\n\n\n";
    std::cout << " R6S pid=" << GetPID("RainbowSix.exe") << " Version = 1.1.1b\n";
    std::cout << " [F1] 发光\n";
    std::cout << " [F2] 肘子发光\n";
    std::cout << " [F3] 切换发光类型\n";
    std::cout << " [F4] 无后座\n";
    ...
    std::cout << " [F9] 无敌\n";
    std::cout << " [F11] 无限子弹\n";
    std::cout << "上面的功能都没用，问就是懒得修，全解锁能用就完事了，其他功能被我注释掉了\n";
    std::cout << " ";
